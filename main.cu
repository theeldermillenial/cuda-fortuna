...
__constant__ unsigned char device_data[67];
...
int main(int argc, char **argv) {
	...
	*/
	...
	unsigned int * host_nonce = new unsigned int[4];
	memset(host_nonce, 0, sizeof(unsigned int) * 4);
	...
	while (true) {
		...
		CUDA_SAFE_CALL(cudaMalloc((void **) &device_nonce, 4 * sizeof(unsigned int)));
		...
		*/
		...
		CUDA_SAFE_CALL(cudaMemcpyToSymbol(device_data, &data[0], 67));
		...
		CUDA_SAFE_CALL(cudaMemcpy(device_nonce, &host_nonce[0], 4 * sizeof(unsigned int), cudaMemcpyHostToDevice));
		...
		CUDA_SAFE_CALL(cudaMemcpy(host_nonce, &device_nonce[0], 4 * sizeof(unsigned int), cudaMemcpyDeviceToHost));
...
__device__ void cuda_sha256_first_update(SHA256_RX *ctx) {
	...
	ctx->bitlen = 512;
	ctx->datalen = 3;
	ctx->data.word[0] = ctx->data.word[16];
}
